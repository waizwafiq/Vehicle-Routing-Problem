...
public class Graph<T extends Comparable<T>, N extends Comparable<N>> {
    ...
    private class Vertex<A extends Comparable<A>, B extends Comparable<B>> {
        private A vertexInfo;
        private int indeg, outdeg; //the number of in-degrees and out-degrees
        private Vertex<A, B> nextVertex; //reference to the next vertex
        private Edge<A, B> firstEdge; //reference to the first edge node
        ...
        public Vertex(A vertexInfo, Vertex<A, B> nextVertex) {
    ...
    private class Edge<A extends Comparable<A>, B extends Comparable<B>> {
        private Vertex<A, B> toVertex; //the vertex the edge points to
        private B weight; //the weight of this edge
        private Edge<A, B> nextEdge; //the next edge node after this (from the same source)

        public Edge() {
        }

        public Edge(Vertex<A, B> toVertex, B weight, Edge<A, B> nextEdge) {
            ...
            if (sourceVertex.vertexInfo.compareTo(sourceInfo) == 0) {
                ...
                while (destinationVertex != null) {
                    if (destinationVertex.vertexInfo.compareTo(destinationInfo) == 0) {
                        ...
                        sourceVertex.firstEdge = new Edge<>(destinationVertex, weight, currentEdge); // add the new edge connected from the source vertex to
                        // destination vertex
                        sourceVertex.outdeg++;
